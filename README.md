...
| 🔨 **Construir y previsualizar (Vite, Next, CRA…)** | El Sandbox ejecuta HTML/CSS/JS tal cual, sin bundler — pero si el proyecto que estás creando es de Vite, Next o CRA, la pestaña Vista muestra una caja con «Construir y previsualizar»: instala dependencias y corre el `build` real en el servidor (sin necesidad de que sea un repo de GitHub) y abre la salida estática (`out/`, `dist/` o `build/`) ahí mismo. Repo Studio (modo descargado) tiene el mismo botón para repos ya clonados. Si el proyecto usa rutas de servidor/SSR de verdad (como la propia Forja), la build se hace igual pero avisa que hace falta un servidor Node — Vercel, un VPS… — para probar esa parte en vivo. |
