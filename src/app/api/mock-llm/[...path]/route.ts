...
const MODELOS = [
  ...
  "mock-tools-free",
  // El tool_call llega troceado en varios deltas SSE (como Groq/OpenRouter):
  // prueba de que el cliente acumula los fragmentos por `index`.
  "mock-tools-fragmentado",
  "mock-cortado",
  "mock-vacio",
...
export async function POST(req: NextRequest, ctx: { params: Promise<{ path: string[] }> }) {
  ...
  // `mock-tools-fragmentado`: el tool_call llega TROCEADO en varios deltas
  // SSE — como lo mandan Groq/OpenRouter de verdad. El primer delta lleva
  // id+nombre con `arguments: ""`, los siguientes añaden fragmentos del JSON
  // por `index`. Si el cliente no acumula por `index`, la llamada llega
  // rota (solo el último fragmento, o tres llamadas a medias).
  if (body.model === "mock-tools-fragmentado" && body.tools && !lastIsToolResult) {
    const argsCompletos = JSON.stringify({
      path: "index.html",
      content: '<!DOCTYPE html><html lang="es"><body><h1>Fragmentado</h1></body></html>',
    });
    const tercio = Math.ceil(argsCompletos.length / 3);
    const fragmentos = [0, 1, 2].map((i) => argsCompletos.slice(i * tercio, (i + 1) * tercio));
    const encoder = new TextEncoder();
    const stream = new ReadableStream({
      start(controller) {
        try {
          // Delta 1: abre la llamada (id + nombre, argumentos vacíos).
          controller.enqueue(
            encoder.encode(
              `data: ${JSON.stringify({
                id: "mock-frag-1",
          ...
          // Deltas 2 y 3: fragmentos del JSON de argumentos, misma `index`.
