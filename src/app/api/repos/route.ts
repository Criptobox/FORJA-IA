...
/* Techo PRÁCTICO por debajo del maxDuration: Vercel mata la función a los
 * ~60 s sin aviso y el cliente solo ve un error de red genérico. Cortando
 * nosotros a los 50 s, el spawnSync en curso aborta controladamente y el
 * mensaje explica qué pasó y qué hacer (construir en local, o un proyecto
 * más pequeño). En desarrollo propio no hay techo de plataforma: ahí los
 * timeout de spawnSync (8 min install / 5 min build) mandan como siempre. */
const TECHO_FUNCION_MS = 50_000;
...
function runInstallAndBuild(dir: string): BuildRunResult {
  ...
  /** Segundos de spawnSync que aún caben sin pasarse del techo de la
   * función (solo importa desplegado en Vercel; en local no resta). */
  const margen = () => Math.max(1, Math.floor((TECHO_FUNCION_MS - (Date.now() - empezó)) / 1000));
  ...
  const install = spawnSync(pmBin(pm), installArgs(pm), {
    cwd: dir,
    encoding: "utf8",
    timeout: Math.min(8 * 60_000, margen() * 1000),
    maxBuffer: 20 * 1024 * 1024,
  });
  if (install.error || install.status !== 0) {
    const seAcabóElTiempo = Date.now() - empezó >= TECHO_FUNCION_MS;
    return {
      kind: "error",
      status: 500,
      error: seAcabóElTiempo
        ? `«${pm} install» se quedó sin tiempo: en despliegues serverless (Vercel) la función tiene un techo de ~50 s y este proyecto necesita más. Construye en local o usa un proyecto más pequeño.`
        : `Falló «${pm} install»`,
  ...
  const build = spawnSync(pmBin(pm), ["run", "build"], {
    cwd: dir,
    encoding: "utf8",
    timeout: Math.min(5 * 60_000, margen() * 1000),
    maxBuffer: 20 * 1024 * 1024,
  });
  ...
  if (build.error || build.status !== 0) {
    ...
    return {
      ...
      status: 500,
      error: seAcabóElTiempo
        ? "«build» se quedó sin tiempo: en despliegues serverless (Vercel) la función tiene un techo de ~50 s y este proyecto necesita más. Construye en local o usa un proyecto más pequeño."
        : `Falló «${pm} run build»`,
      log: buildLog,
    };
  }
