...
import { accessCodeHeaders, fetchModels } from "@/lib/forja/chat-client";
...
import {
  ...
  resumenNovedades,
  type NovedadGratis,
} from "@/lib/forja/radar-propio";
import {
  compararFoto,
  nuevaFoto,
  resumenCambio,
  type ResultadoCambio,
} from "@/lib/forja/cambio-gratis";
import {
  RADAR_NOVEDAD_IDS,
  RADAR_OFFERS,
  RADAR_PAGES,
  ...
  type RadarOffer,
  type RadarSource,
} from "@/lib/forja/free-radar";
import { frescuraDe, pideRevision, textoFrescura } from "@/lib/forja/frescura";
...
function hasOpenRouterKey(): boolean {
  return !!useForja.getState().providers.openrouter.apiKey.trim();
}
...
export function FreeRadarDialog({
  ...
  const markRadarSeen = useForja((s) => s.markRadarSeen);
  const addModelToProvider = useForja((s) => s.addModelToProvider);
  ...
  const buscarEnTusProveedores = useCallback(async () => {
    ...
    try {
      ...
      useForja.getState().setFotoGratis(nuevaFoto(foto, catalogo));
    } finally {
    ...
    (src: RadarSource) => {
      if (src.providerId) {
        ...
        for (const m of src.models) {
          if (useForja.getState().addModelToProvider(src.providerId, m)) addedCount++;
        }
        const cfg = useForja.getState().providers[src.providerId];
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="flex h-[88vh] max-w-2xl flex-col gap-0 overflow-hidden p-0 sm:h-[640px]">
        ...
        <div className="min-h-0 flex-1 space-y-5 overflow-y-auto px-4 py-4">
                          ...
                          <button
                            ...
                            className={cn(
                              "max-w-full truncate rounded-full border px-2 py-0.5 text-[10px] transition",
                                ...
                                : "border-border bg-secondary/60 text-secondary-foreground hover:border-forja-violet/40 hover:text-forja-violet"
                            )}
                          >
                            {done ? "✓ " : "+ "}
                            {m}
                      ...
                      <button
                        ...
                        className={cn(
                          "flex size-6 shrink-0 items-center justify-center rounded-full border transition",
                            ...
                            : "border-border text-muted-foreground hover:border-forja-violet/40 hover:text-forja-violet"
                        )}
                      >
                        {done ? <Check className="size-3" /> : <Plus className="size-3" />}
                      </button>
          ...
          <section>
            <h3 className="mb-2 flex items-center gap-1.5 px-1 text-[11px] font-semibold uppercase tracking-wide text-muted-foreground">
              <Radar className="size-3 text-forja-cyan" /> En vivo · OpenRouter :free
              {data && (
                <span
                  className={cn(
                    "rounded-full px-1.5 py-px text-[9px] font-semibold",
            ...
            {data && !loading && (
              <ul className="divide-y rounded-xl border border-border/60 bg-card/50">
                {data.openrouter.map((m) => {
                  ...
                  const _hasOpenRouterKey = !!useForja.getState().providers.openrouter.apiKey.trim();
                  return (
            ...
            {data?.live && !hasOpenRouterKey() && (
              <p className="mt-1.5 flex flex-wrap items-center gap-1.5 px-1 text-[10.5px] text-muted-foreground/70">
                ...
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="inline-flex items-center gap-1 rounded-full border border-forja-violet/40 px-2 py-0.5 font-medium text-forja-violet transition hover:bg-forja-violet/10"
                >
          ...
          <section>
            <h3 className="mb-2 flex items-center gap-1.5 px-1 text-[11px] font-semibold uppercase tracking-wide text-muted-foreground">
              <ExternalLink className="size-3 text-forja-violet" /> Páginas para estar al tanto
            </h3>
              ...
              {RADAR_PAGES.map((p) => (
                <li key={p.id}>
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="flex items-start gap-2 rounded-xl border border-border/60 bg-card/50 px-3.5 py-2.5 transition hover:border-forja-violet/40"
                  >
                    <div className="min-w-0 flex-1">
