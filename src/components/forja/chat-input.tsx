...
import { startDictation, speechToTextSupported } from "@/lib/forja/speech";
import { useForja } from "@/lib/forja/store";
import { calcularHud, fmtTokens, type NivelCtx } from "@/lib/forja/ctx-hud";
import { resolverPlantillaMarketing } from "@/lib/forja/marketing";
import { ultimoDiseno as ultimoDisenoDe, leerMemoria } from "@/lib/forja/memoria-proyecto";
...
import {
  ...
  slashQuery,
  type SlashCommand,
} from "@/lib/forja/slash";
import type { Attachment, DocText } from "@/lib/forja/types";
...
export function ChatInput({
  ...
  const outputStyle = useForja((st) => st.settings.outputStyle);
  const setSettings = useForja((st) => st.setSettings);
  const ESTILOS = [
  ...
  return (
    <div className="safe-bottom pointer-events-auto sticky bottom-0 z-10 px-3 pb-3 pt-2 sm:px-6">
      <div className="mx-auto max-w-3xl">
        ...
        <div
          className={cn(
            "glass flex flex-col gap-1.5 rounded-2xl border border-border/80 p-2 shadow-lg shadow-black/[0.06] transition focus-within:border-forja-violet/50 dark:shadow-black/30",
            disabled && "opacity-60"
          )}
        >
          ...
          {hasDocs && (
            <div className="flex gap-2 overflow-x-auto px-1 pb-0.5 pt-1">
              {docs.map((d) => (
                ...
                >
                  <FileText className="size-4 text-forja-cyan" />
          ...
          {toolsOpen && (
            ...
            >
              ...
              <Tool
                ...
                disabled={streaming}
                pressed={!!agent}
                activeClass="bg-forja-violet/10 text-forja-violet hover:bg-forja-violet/15 hover:text-forja-violet"
              ...
              <Tool
                ...
                disabled={streaming}
                pressed={!!consensus}
                activeClass="bg-forja-cyan/10 text-forja-cyan hover:bg-forja-cyan/15 hover:text-forja-cyan"
              ...
              <Tool
                ...
                disabled={streaming}
                pressed={!!imageMode}
                activeClass="bg-forja-pink/10 text-forja-pink hover:bg-forja-pink/15 hover:text-forja-pink"
          ...
          <div className="flex items-end gap-1.5">
            ...
            <button
              ...
              aria-controls="forja-chat-tools"
              ...
              className={cn(
                ...
                toolsOpen && "border-forja-violet/40 bg-muted text-foreground",
                hayModo && "ring-2 ring-forja-violet/35"
              )}
            >
              <Plus className={cn("size-4 transition-transform duration-200", toolsOpen && "rotate-45")} />
              {hayModo && (
                <span className="absolute right-1 top-1 flex gap-0.5">
                  {agent && <span className="size-1.5 rounded-full bg-forja-violet" />}
                  {imageMode && <span className="size-1.5 rounded-full bg-forja-pink" />}
                  {consensus && <span className="size-1.5 rounded-full bg-forja-cyan" />}
            ...
            <textarea
              ...
              rows={1}
              disabled={disabled}
              aria-controls={slashOpen ? "forja-slash-menu" : undefined}
            ...
            {streaming ? (
              ...
              <Button
                ...
                onClick={onSend}
                disabled={!canSend}
                className="forja-gradient-bg size-9 shrink-0 rounded-xl border-0 text-white shadow-md shadow-violet-500/20 hover:opacity-90 disabled:opacity-40"
                title="Enviar (Enter)"
