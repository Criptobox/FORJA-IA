 ...
 *
 * Muestra el informe semanal calculado por `lib/forja/wrapped.ts` a
...
import { useUsage, fmtMs, fmtChars } from "@/lib/forja/usage";
import { computeWrapped, ahorroPct, wrappedToHtml } from "@/lib/forja/wrapped";
...
export function WrappedDialog({
  ...
  const descargar = () => {
    ...
    a.href = url;
    a.download = `forja-wrapped-${new Date().toISOString().slice(0, 10)}.html`;
      ...
      <DialogContent className="max-h-[90vh] overflow-hidden rounded-2xl sm:max-w-2xl">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            <Sparkles className="size-4 text-forja-pink" />
                  ...
                  <div className="flex items-end gap-1.5 rounded-xl border border-border/60 bg-card/40 p-3" style={{ height: 120 }}>
                    {diasOrdenados.map(([k, v]) => (
                      <div key={k} className="flex h-full flex-1 flex-col justify-end gap-1" title={`${k}: ${v} peticiones`}>
                        <div
                          className="w-full rounded-t bg-gradient-to-t from-forja-violet to-forja-cyan"
                      ...
                      <tbody>
                        {stats.ranking.map((r, i) => (
                          <tr key={r.modelKey} className="border-t border-border/40">
                            ...
                            <td className="px-3 py-2 font-mono text-[11px] text-forja-cyan">{r.label}</td>
...
function StatBig({ value, label, sub }: { value: string; label: string; sub?: string }) {
  return (
    <div className="rounded-2xl border border-border/60 bg-card/40 p-3">
      <div className="forja-gradient-text text-[22px] font-bold leading-none">{value}</div>
