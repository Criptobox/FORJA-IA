...
import { permitido, quedaAlguno, alternarVeto } from "@/lib/forja/vetados";
import { cn } from "@/lib/utils";
import { PROVIDERS } from "@/lib/forja/providers";
import { fetchModels, probeModel } from "@/lib/forja/chat-client";
import {
  ...
  probeAll,
  type ProbeResult,
} from "@/lib/forja/model-probe";
import { isFreeModel, sanearOrdenFallback } from "@/lib/forja/free-models";
import { useModelosRotos } from "@/lib/forja/modelos-rotos";
import { sugerirModelos } from "@/lib/forja/sugeridos";
import {
  isNvidiaCatalogPaste,
  looksLikeProviderSnippet,
  parseModelPaste,
  parseProviderSnippet,
} from "@/lib/forja/model-paste";
import { useForja } from "@/lib/forja/store";
import { useVault } from "@/lib/forja/vault";
import { makeModelKey, type ProviderConfig, type ProviderId } from "@/lib/forja/types";
...
function vacio(_id: ProviderId, defModels: string[]): ProviderConfig {
  return {
    apiKey: "",
    enabled: false,
    models: [...defModels],
...
function OrdenFallback() {
  const guardado = useForja((s) => s.fallbackOrder);
  const setFallbackOrder = useForja((s) => s.setFallbackOrder);
  ...
  return (
    <div className="rounded-xl border border-border/60 bg-card/40">
      ...
      >
        <ListOrdered className="size-4 shrink-0 text-forja-violet" />
...
export function ProvidersTab({
  ...
  const setProviderConfig = useForja((s) => s.setProviderConfig);
  ...
  const vetados = useForja((s) => s.settings.proveedoresVetados) ?? [];
  ...
  const pingProvider = async (id: ProviderId, overwrite: boolean) => {
    ...
    const cfg = useForja.getState().providers[id] ?? vacio(id, def.defaultModels);
    ...
    try {
      ...
      if (!models.length) {
        ...
        const onlyFree = useForja.getState().settings.onlyFree;
        toast.success(
          onlyFree
  ...
  return (
    <div className="space-y-3">
      <p className="px-0.5 text-xs leading-relaxed text-muted-foreground">
        ...
        modelo.{" "}
        <span className="font-medium text-forja-violet">NVIDIA NIM</span>,{" "}
        <span className="font-medium text-forja-violet">Kimi</span> y{" "}
        <span className="font-medium text-forja-violet">TokenRouter</span> van nativos.
      </p>
      ...
      <div className="space-y-2 rounded-xl border border-emerald-500/25 bg-emerald-500/[0.06] px-3 py-2.5">
        ...
        <textarea
          ...
          className="w-full resize-y rounded-lg border border-input bg-background px-3 py-2 font-mono text-[11px] leading-relaxed outline-none focus:border-forja-violet/50"
          autoCapitalize="off"
          autoCorrect="off"
          spellCheck={false}
        />
          ...
          {(
            ...
            <button
              ...
              className={cn(
                ...
                filtro === f.id
                  ? "border-forja-violet/50 bg-forja-violet/10 text-foreground"
            ...
            return (
              <button
                ...
                className={cn(
                  ...
                  expanded === id
                    ? "border-forja-violet/50 bg-forja-violet/10"
                    : "border-border/70 bg-card/60 hover:border-forja-violet/30"
                )}
              >
                <span
        ...
        return (
          <div
            ...
            className={cn(
              ...
              isOpen ? "border-forja-violet/40 bg-card" : "border-border/60 bg-card/40",
              def.featured && "ring-1 ring-forja-violet/20"
            )}
          >
            <div className="flex min-h-12 items-center gap-2.5 px-3 py-2.5">
              ...
              <button onClick={() => toggleExpand(def.id)} className="min-w-0 flex-1 text-left">
                <p className="flex flex-wrap items-center gap-1.5 text-[13px] font-medium">
                  {def.name}
                  {def.featured && (
                    <span className="rounded-full bg-forja-violet/15 px-1.5 py-px text-[9.5px] font-semibold uppercase tracking-wide text-forja-violet">
                      1 clave = todo
                    </span>
                  )}
                  {listo && (
                      ...
                      <div className="flex flex-wrap gap-1.5">
                        {sugerencias.modelos.map((m) => (
                          <button
                            ...
                            className={cn(
                              "max-w-full truncate rounded-full border bg-background px-2.5 py-1 text-left font-mono text-[11px] hover:border-forja-violet/40 hover:text-forja-violet",
                              sugerencias.origen === "catalogo"
