...
import { reglaFromDiagnostico, useFailures } from "@/lib/forja/failures";
import {
  compareRuns,
  comparables,
  type RegressionDiff,
  type RunSnapshot,
} from "@/lib/forja/regression";
import {
  ejecutarPasosPiloto,
  informePiloto,
  injectPilot,
          ...
          return (
            <li key={`d:${node.path}`}>
              ...
              >
                ...
                />
                {isOpen ? (
                  <FolderOpen className="size-3.5 shrink-0 text-forja-cyan" />
                ) : (
                  <FolderClosed className="size-3.5 shrink-0 text-forja-cyan/70" />
                )}
      ...
      >
        <DialogHeader className={cn("border-b pb-3 pt-4", pantallaCompleta ? "px-3" : "px-5")}>
          <DialogTitle className="flex items-center gap-2 pr-8 text-base">
            <Box className="size-4 shrink-0 text-forja-cyan" /> Sandbox
            {name && (
        ...
        {!paths.length ? (
          ...
          <div className="flex min-h-0 flex-1 flex-col items-center justify-center gap-4 overflow-y-auto p-6">
            <div
              ...
              onDragOver={(e) => {
                e.preventDefault();
                dragRef.current?.classList.add("ring-2", "ring-forja-cyan");
              }}
              onDragLeave={() => dragRef.current?.classList.remove("ring-2", "ring-forja-cyan")}
              onDrop={(e) => {
                e.preventDefault();
                dragRef.current?.classList.remove("ring-2", "ring-forja-cyan");
                    ...
                    {pilotoAbierto && (
                      <div className="shrink-0 border-b border-border/60 bg-muted/30 px-3 py-2">
                        ...
                        <textarea
                          ...
                          className="w-full resize-y rounded-md border border-border/60 bg-background p-2 font-mono text-[11px] leading-relaxed outline-none placeholder:text-muted-foreground/50 focus:ring-1 focus:ring-forja-cyan/50"
