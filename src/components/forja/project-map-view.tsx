...
import { fileRelations } from "@/lib/forja/project-map";
import type { ProjectMap } from "@/lib/forja/types";
import { cn } from "@/lib/utils";
import { ProjectGraph } from "./project-graph";
import { ProjectPassportCard } from "./project-passport";
import { afectados, validarPatron, MAX_REGLAS, type ReglaNo } from "@/lib/forja/reglas-no";

const VIEW_KEY = "forja-map-view-v1";
...
function kindIcon(_kind: string) {
  return <FileCode2 className="size-3.5 shrink-0 text-forja-cyan" />;
}
  ...
  return (
    <div className="flex h-full flex-col">
      ...
      <div className="flex items-start gap-3 border-b border-border/60 p-3">
        <div className="mt-0.5 flex size-8 shrink-0 items-center justify-center rounded-xl forja-gradient-bg text-white">
                          ...
                          <div className="mt-1.5 space-y-0.5 pl-6 font-mono text-[10px] leading-relaxed text-muted-foreground">
                            {rel.outgoing.length > 0 && (
                              <p className="truncate">
                                <span className="text-forja-cyan">→</span> {rel.outgoing.join(", ")}
                              </p>
                            )}
