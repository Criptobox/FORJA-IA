...
import { makeModelKey, splitModelKey, type ProviderId } from "@/lib/forja/types";
import { useForja } from "@/lib/forja/store";
import { isFreeModel } from "@/lib/forja/free-models";
import { estadoModelo, MODELOS_FECHA } from "@/lib/forja/modelos-viejos";
import { estaRoto, useModelosRotos } from "@/lib/forja/modelos-rotos";
...
export function useAvailableModels(conservar?: string | null): ModelOption[] {
  ...
  const favorites = useForja((s) => s.favorites);
  const onlyFree = useForja((s) => s.settings.onlyFree);
...
export function ModelPicker({
  ...
  const favorites = useForja((s) => s.favorites);
  const toggleFavorite = useForja((s) => s.toggleFavorite);
  const [query, setQuery] = useState("");

  // Atajo global Ctrl+K: abre el selector (evento lanzado desde chat-app)
  useEffect(() => {
    const openPicker = () => setOpen(true);
    window.addEventListener("forja-open-model-picker", openPicker);
    return () => window.removeEventListener("forja-open-model-picker", openPicker);
  }, []);
  ...
  const addCustom = () => {
    ...
    const split = splitModelKey(value);
    if (!split) return;
    const cfg = useForja.getState().providers[split.providerId];
    if (cfg && !cfg.models.includes(qq)) {
      useForja.getState().setProviderConfig(split.providerId, {
        ...
        >
          {selectedInfo || isAutoKey(value) || isForjaWebKey(value) ? (
            ...
            <span className="flex items-center gap-2 text-muted-foreground">
              <Sparkles className="size-4 text-forja-cyan" />
                ...
                {filtered.map((m) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {value === m.key && <Check className="size-4 text-forja-cyan" />}
                    </CommandItem>
                  );
                })}
              </CommandGroup>
