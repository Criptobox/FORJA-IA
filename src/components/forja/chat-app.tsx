...
import { abrirTab, cerrarTab } from "@/lib/forja/tabs";
import type { PublishSeed, SandboxSeed } from "@/lib/forja/sandbox";
import { conKit } from "@/lib/forja/efectos";
...
import { useForja, uid } from "@/lib/forja/store";
import { migrateLegacyAttachments } from "@/lib/forja/attachment-blob";
import { useChatAttachments } from "@/lib/forja/use-chat-attachments";
import { useGeneration } from "@/lib/forja/use-generation";
import { mereceOrquesta } from "@/lib/forja/orquesta";
import { skillsSugeridas, textoSugerencia } from "@/lib/forja/skills-sugeridas";
import { anchorAt } from "@/lib/forja/branches";
import { ThreadBar } from "./thread-bar";
import { extractPreviewHtml } from "@/lib/forja/preview";
import {
  DEMO_MODEL,
  DEMO_PROMPT,
  DEMO_TITLE,
  ...
  demoReply,
  typeDemoReply,
} from "@/lib/forja/preview-demo";
import { useSystemPrompt } from "@/lib/forja/use-system-prompt";
import { estimarTokensConversacion, ventanaReferencia } from "@/lib/forja/ctx-hud";
import { useLimites } from "@/lib/forja/limites-medidos";
import { agentStalled, continuePrompt, parseAgentTrace, suggestAgentMode } from "@/lib/forja/agent-loop";
import { applyAccent } from "@/lib/forja/accent";
import {
  downloadSessionHtml,
  downloadSessionMarkdown,
  printSessionPdf,
} from "@/lib/forja/export-chat";
import { stopSpeaking } from "@/lib/forja/speech";
import { recapPrompt, translatePrompt, type TargetLang } from "@/lib/forja/recap";
import { useFocusMode } from "@/lib/forja/focus-mode";
import type { SlashCommand } from "@/lib/forja/slash";
import { initVault, useVault } from "@/lib/forja/vault";
import {
...
import {
  ...
  pickManualModel,
  type ProviderId,
} from "@/lib/forja/types";
import { PROVIDER_MAP } from "@/lib/forja/providers";
import { esTurnoTrivial } from "@/lib/forja/turno-trivial";
import { classifyTask } from "@/lib/forja/task-router";
import { useHealth } from "@/lib/forja/health";
import { resumenCuota, tonoCuota, useQuota } from "@/lib/forja/quota";
import { useUsage } from "@/lib/forja/usage";
import { unseenRadarCount } from "@/lib/forja/free-radar";
import { extractRepoFromText, isMostlyRepoLink } from "@/lib/forja/repo-cloud";
...
} from "@/lib/forja/auto-contexto";
import { reglaQueBloquea } from "@/lib/forja/reglas-no";
import { Dialog, DialogContent, DialogHeader, DialogTitle, DialogDescription, DialogFooter } from "@/components/ui/dialog";
import { SnapshotsPanel } from "./snapshots-panel";
import { RepasoDialog } from "./repaso-dialog";
import { OfertasDialog } from "./ofertas-dialog";
import { useRepaso } from "@/lib/forja/repaso-store";
import { useOfertas } from "@/lib/forja/ofertas-store";
import { extraerTarjetas, fechaHoy, PROMPT_REPASO, resumenRepaso } from "@/lib/forja/repaso";
import { fusionarOfertas, novedadesOfertas, OFERTAS_BASE } from "@/lib/forja/ofertas";
import { MemoriaPanel } from "./memoria-panel";
import { ForjaStudioDialog } from "./forja-studio-dialog";

export function ChatApp() {
  // hidratación
  const hydrated = useForja((s) => s.hydrated);
  ...
  const branchFrom = useForja((s) => s.branchFrom);
  const startThread = useForja((s) => s.startThread);
  const switchThread = useForja((s) => s.switchThread);
  const removeThread = useForja((s) => s.removeThread);
  const renameThread = useForja((s) => s.renameThread);
  const switchBranch = useForja((s) => s.switchBranch);
  const createSession = useForja((s) => s.createSession);
  const renameSession = useForja((s) => s.renameSession);
  const setActiveSession = useForja((s) => s.setActiveSession);
  ...
  const clearMessages = useForja((s) => s.clearMessages);
  const setProjectMap = useForja((s) => s.setProjectMap);
  const addReglaNo = useForja((s) => s.addReglaNo);
  const removeReglaNo = useForja((s) => s.removeReglaNo);
  ...
  const radarSeenIds = useForja((st) => st.radarSeenIds);
  ...
  // el contenido en una cookie efímera `forja-share` y redirige a
  ...
  useEffect(() => {
    ...
    try {
      const raw = document.cookie
        .split("; ")
        .find((c) => c.startsWith("forja-share="));
      if (!raw) return;
      const contenido = decodeURIComponent(raw.slice("forja-share=".length));
      if (!contenido) return;
      setInput(contenido);
      // Borra la cookie: es de un solo uso.
      document.cookie = "forja-share=; path=/; max-age=0; SameSite=Lax";
  ...
  useEffect(() => {
    if (!hydrated) return;
    if (useForja.getState().onboardingDone) return;
    const t = setTimeout(() => {
      if (!useForja.getState().onboardingDone) setOnboardingOpen(true);
    }, 600);
  ...
  useEffect(() => {
    ...
    let already = false;
    try {
      already = localStorage.getItem("forja-preview-demo") === "1";
    } catch {
    ...
    if (existing) {
      ...
      createSession();
      sessionId = useForja.getState().ensureSession();
        ...
        for (const sug of sugerencias) {
          ...
          toast(`${sug.skill.icon} ${sug.skill.name}`, {
            ...
            action: {
              label: "Activar",
              onClick: () => {
                useForja.getState().toggleSkill(sug.skill.id);
  ...
  const handleSlash = useCallback(
    (cmd: SlashCommand) => {
      switch (cmd.id) {
        ...
        case "agente": {
          const on = !useForja.getState().settings.agentMode;
   ...
   * (bloque forja-repaso). Las propuestas ya vienen filtradas y deduplicadas
  ...
  useEffect(() => {
    const handler = (e: KeyboardEvent) => {
      ...
      if (mod && !e.shiftKey && e.key.toLowerCase() === "k") {
        e.preventDefault();
        window.dispatchEvent(new CustomEvent("forja-open-model-picker"));
        return;
      }
    ...
    <main className="relative flex h-full min-w-0 flex-1 flex-col">
      ...
      <header className="glass sticky top-0 z-20 flex h-14 items-center gap-2 border-b border-border/60 px-3 sm:px-4">
        ...
        <Button
          ...
          className={cn(
            ...
            isAutoKey(modelKey) && "forja-gradient-bg border-0 text-white hover:opacity-90"
          )}
          onClick={() => {
        ...
        {mobilePreviewReady && (
          ...
          <Button
            size="sm"
            className="h-8 shrink-0 gap-1.5 px-2.5 text-xs lg:hidden forja-gradient-bg border-0 text-white hover:opacity-90"
        ...
        <Button
          variant="ghost"
          size="icon"
          className={cn("hidden size-8 lg:inline-flex", focusMode && "text-forja-violet")}
          onClick={() => {
            toggleFocusMode();
      ...
      <div
        ...
        style={{
          ...
          background:
            "linear-gradient(160deg, color-mix(in oklab, var(--forja-violet) 6%, transparent) 0%, transparent 38%, color-mix(in oklab, var(--forja-cyan) 5%, transparent) 100%)",
        }}
      >
        {!hasMessages ? (
          <Welcome
