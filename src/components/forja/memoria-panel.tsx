 ...
 *
 * Y la pieza que lo ata al repo: exportar a los cinco JSON de `.forja/` para
...
export function MemoriaPanel({
  ...
  const exportar = () => {
    if (!sessionId) return;
    const archivos = aArchivosForja(memoria);
    ...
    void navigator.clipboard
      ...
      .then(() =>
        toast.success("Memoria copiada", {
          description: `${nombres.length} archivo(s) .forja/ listos para el repo: ${nombres.join(", ")}`,
        })
      )
      ...
      <DialogContent className="max-h-[85vh] w-[min(94vw,620px)]">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            <Brain className="size-4 text-forja-violet" /> Memoria del proyecto
          </DialogTitle>
          <DialogDescription>
            {sesionTitulo ? `«${sesionTitulo}» — ` : ""}Lo que Forja sabe de este
            proyecto: decisiones, errores con su solución, tareas hechas y estilos
            ya usados. Viaja con el proyecto si exportas la memoria a `.forja/`.
          </DialogDescription>
        </DialogHeader>
                ...
                memoria.disenos.map((d) => (
                  <div key={d.id} className="rounded-lg border border-border/60 bg-card/60 p-2.5 text-[12px]">
                    <p className="flex items-center gap-1.5 font-medium">
                      <Palette className="size-3.5 text-forja-violet" />
                      {d.direccion}
                    </p>
        ...
        <div className="flex items-center justify-between gap-2">
          <div className="flex items-center gap-2">
            <Button variant="outline" size="sm" className="h-7 gap-1.5 text-xs" onClick={exportar}>
              <FileCode2 className="size-3" /> Exportar .forja/
            </Button>
            <Button
              variant="ghost"
              size="sm"
        ...
        <p className="text-[10.5px] text-muted-foreground/70">
          ...
          .forja/ del repo (Download arriba)
        </p>
      </DialogContent>
    </Dialog>
  );
