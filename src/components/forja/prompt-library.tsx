...
import { PROMPT_CATEGORIES } from "@/lib/forja/prompts-data";
...
export function PromptLibrary({
  ...
  const prompts = useForja((s) => s.prompts);
  const addPrompt = useForja((s) => s.addPrompt);
  const deletePrompt = useForja((s) => s.deletePrompt);
      ...
      <DialogContent className="flex h-[86vh] max-w-xl flex-col gap-0 overflow-hidden p-0 sm:h-[600px]">
        <DialogHeader className="border-b px-5 py-4">
          <DialogTitle className="flex items-center gap-2 text-base">
            <BookOpen className="size-4 text-forja-cyan" /> Biblioteca de prompts
          </DialogTitle>
          ...
          <div className="flex flex-wrap gap-1.5">
            <button
              ...
              className={cn(
                ...
                category === null
                  ? "border-forja-violet/50 bg-forja-violet/10 text-forja-violet"
          ...
          {filtered.length === 0 ? (
            ...
            <ul className="space-y-1.5">
              {filtered.map((p) => (
                <li
                  key={p.id}
                  className="group flex cursor-pointer items-start gap-2.5 rounded-xl border border-border/50 bg-card/50 px-3 py-2.5 transition hover:border-forja-violet/40 hover:bg-card"
                  onClick={() => {
                    onPick(p.content);
                    onOpenChange(false);
                  }}
