...
import {
  ...
  toolsDelEfecto,
  toolPermitida,
} from "@/lib/forja/tool-permissions";
import { TOOL_CATALOG } from "@/lib/forja/tools-catalog";
import {
  normalizarTope,
  TOPE_DIARIO_POR_DEFECTO,
  TOPE_MINIMO,
  TOPE_MAXIMO,
} from "@/lib/forja/gasto";
import {
  Dialog,
  DialogContent,
  DialogDescription,
...
import { ACCENTS, ACCENT_CUSTOM, normalizeHex } from "@/lib/forja/accent";
import { useForja } from "@/lib/forja/store";
import { MODOS_AGENTE, costeDeModos } from "@/lib/forja/agent-modes";
import {
  CHARS_POR_TOKEN,
  construirPrompt,
  nivelPresupuesto,
  piezaMasGorda,
  tokensAprox,
} from "@/lib/forja/presupuesto";
import { entradaPromptActual } from "@/lib/forja/prompt-actual";
import { APP_BUILT, APP_COMMIT, APP_VERSION, buildLabel } from "@/lib/forja/app-version";
...
export function SettingsDialog({
  ...
  const exportData = useForja((s) => s.exportData);
  const importData = useForja((s) => s.importData);
  const resetAll = useForja((s) => s.resetAll);
          ...
          <TabsContent value="chat" className="min-h-0 flex-1 space-y-5 overflow-y-auto px-5 py-4">
            <div className="space-y-1.5">
              ...
              <textarea
                ...
                rows={4}
                className="w-full resize-none rounded-lg border border-input bg-background px-3 py-2 text-sm outline-none focus:border-forja-violet/50"
                  ...
                  <button
                    ...
                    className={cn(
                      ...
                      settings.compression === o.v
                        ? "border-forja-violet/60 bg-forja-violet/10 text-foreground"
            ...
            <div className="flex items-center justify-between rounded-xl border border-border/60 px-4 py-3">
              <div>
                <Label className="flex items-center gap-1.5 text-[13px]">
                  <Volume2 className="size-3.5 text-forja-violet" /> Leer respuestas en voz alta
                </Label>
                <p className="text-[11px] leading-relaxed text-muted-foreground">
            ...
            <div className="space-y-1.5 rounded-xl border border-border/60 px-4 py-3">
              ...
              />
              <p className="text-[11px] leading-relaxed text-muted-foreground">
                Si tu despliegue (Vercel, VPS…) define la variable <code className="rounded bg-muted px-1">FORJA_ACCESS_CODE</code>,
                  ...
                  return (
                    <button
                      ...
                      className={cn(
                        ...
                        active
                          ? "border-transparent ring-2 ring-forja-violet"
                    ...
                    >
                      ...
                      />
                      {a.name}
                      {active && <Check className="ml-auto size-3.5 text-forja-violet" />}
                    </button>
                  );
                })}
              </div>
              ...
              <div className="mt-3 flex flex-wrap gap-2">
                <Button
                  ...
                  onClick={() => {
                    ...
                    a.href = url;
                    a.download = `forja-ai-backup-${new Date().toISOString().slice(0, 10)}.json`;
                    a.click();
                ...
                <Button
                  ...
                  onClick={() => {
                    ...
                    const a = document.createElement("a");
                    a.href = url;
                    a.download = `forja-ai-ajustes-${new Date().toISOString().slice(0, 10)}.json`;
                    a.click();
                    URL.revokeObjectURL(url);
...
function AppearanceCustom() {
   ...
   * versión anterior no traen el campo, y un efecto nuevo se concede. */
  const permisos = useMemo(() => normalizarPermisos(settings.permisosAgente), [settings.permisosAgente]);
  const _apagados = useMemo(() => EFECTOS.filter((e) => !permisos[e]), [permisos]);
  /** Qué herramientas quedan fuera con lo apagado ahora mismo. Se calcula del
   * catálogo real: es la única forma de que la advertencia no mienta cuando
   * se añada una herramienta nueva. */
  const _sinPermiso = useMemo(
    () => TOOL_CATALOG.filter((t) => !toolPermitida(t.name, permisos).permitida).map((t) => t.name),
    [permisos]
  );
  ...
  return (
    <div
      className={cn(
        ...
        isCustom ? "border-forja-violet/40 bg-forja-violet/5" : "border-border/60"
      )}
    >
      <Label className="flex items-center gap-1.5 text-[13px]">
        <Palette className="size-3.5 text-forja-violet" /> Color personalizado
      </Label>
      <div className="flex items-center gap-2">
        ...
        />
        {isCustom && (
          <span className="ml-auto inline-flex items-center gap-1 text-[11px] font-medium text-forja-violet">
...
function VaultCard() {
  ...
  return (
    <div className="rounded-xl border border-border/60 p-4">
      <h3 className="flex items-center gap-1.5 text-sm font-semibold">
        <LockKeyhole className="size-4 text-forja-violet" /> Seguridad · Cifrado con PIN
        {enabled && (
...
function MedidorPrompt() {
  const settings = useForja((s) => s.settings);
  const skills = useForja((s) => s.skills);
  const activeSessionId = useForja((s) => s.activeSessionId);
