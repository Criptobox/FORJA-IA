 ...
 * acento `forja-violet`), usa el icono oficial del proyecto y el
    ...
    <div className="group/code overflow-hidden rounded-lg border border-border/70 bg-[#0B0E17] text-[#E6E9F2] shadow-sm">
      <div className="flex items-center justify-between border-b border-white/[0.07] bg-white/[0.03] px-3 py-1.5">
        <div className="flex min-w-0 items-center gap-2">
          <FileCode2 className="size-3.5 shrink-0 text-forja-cyan" />
          {filename && (
  ...
  return (
    <div className="agent-trace-v316 space-y-2">
      ...
      <div className="agent-trace-header flex items-center gap-2">
        <ForjaLogo size={18} className="text-forja-violet" />
        <span className="text-[11px] font-semibold uppercase tracking-wide text-forja-violet">
          Bucle del agente
        </span>
        {streaming && (
          <span className="ml-auto flex items-center gap-1.5 text-[11px] text-muted-foreground">
            <Loader2 className="size-3 animate-spin text-forja-cyan" />
          ...
          <ul className="space-y-1">
            {plan.items.map((item, i) => (
              <li key={i} className="flex gap-2 text-[13px] leading-snug">
                <span className="mt-[7px] size-1.5 shrink-0 rounded-full bg-forja-violet/60" />
          ...
          <ul className="space-y-1.5">
            {files.map((f, i) => (
              <li key={i} className="flex items-start gap-2 text-[12.5px]">
                <FileCode2 className="mt-0.5 size-3.5 shrink-0 text-forja-cyan" />
                <div className="min-w-0">
                  <p className="truncate font-medium">{f.name}</p>
                  {f.summary && <p className="truncate text-muted-foreground">{f.summary}</p>}
                </div>
                <span className="ml-auto shrink-0 rounded-md bg-forja-violet/10 px-1.5 py-0.5 text-[10px] uppercase tracking-wide text-forja-violet">
                  {f.kind}
                </span>
              </li>
            ))}
              ...
              c.blocks.length === 0 ? null : (
                <div key={i} className="space-y-2">
                  <div className="flex items-center gap-2 px-0.5">
                    <span className="inline-flex shrink-0 items-center gap-1 rounded-md bg-forja-cyan/10 px-1.5 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-forja-cyan">
              ...
              <ul className="space-y-1.5">
                {iterations.map((it, i) => {
                  ...
                  return (
                    <li key={i} className="flex items-start gap-2 text-[12.5px]">
                      {running ? (
                        <Loader2 className="mt-0.5 size-3.5 shrink-0 animate-spin text-forja-cyan" />
                      ) : it.review?.pass ? (
                        <CheckCircle2 className="mt-0.5 size-3.5 shrink-0 text-emerald-500" />
                      ) : it.review ? (
                        <RefreshCcw className="mt-0.5 size-3.5 shrink-0 text-amber-500" />
                      ) : (
                        <span className="mt-1 size-1.5 shrink-0 rounded-full bg-forja-violet/60" />
                      )}
  ...
  return (
    <button
      ...
      className={cn(
        ...
        active
          ? "bg-forja-violet text-white shadow-sm"
