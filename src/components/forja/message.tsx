...
import { hayFicha, lineasDeFicha, titularDeFicha } from "@/lib/forja/ficha-respuesta";
import { agentStalled, parseAgentTrace } from "@/lib/forja/agent-loop";
import { proyectoDeLaRespuesta } from "@/lib/forja/auto-revision";
import { citasDe } from "@/lib/forja/evidencia";
import { instructionLabel, TRANSLATE_LANGS, type TargetLang } from "@/lib/forja/recap";
import { useAvailableModels } from "@/components/forja/model-picker";
...
import { speak, stopSpeaking } from "@/lib/forja/speech";
import { PROVIDER_MAP } from "@/lib/forja/providers";
import { resolveAttachmentDataUrl } from "@/lib/forja/attachment-blob";
import type { Attachment } from "@/lib/forja/types";
import { extraerTarjetas } from "@/lib/forja/repaso";
...
export const MessageItem = memo(function MessageItem({
   ...
   * forja-repaso). Solo llega si el contenido trae tarjetas. */
    ...
    return (
      <div data-role="user" className="msg-in group flex flex-col items-end gap-1">
        <div className="flex max-w-[85%] items-end gap-2 sm:max-w-[78%]">
          {editing ? (
            <div className="flex w-full min-w-0 flex-col gap-2 rounded-2xl border border-forja-violet/40 bg-card p-3">
              <textarea
                value={draft}
            ...
            <>
              ...
              <div className="tint-user rounded-2xl rounded-br-md border border-forja-violet/20 px-4 py-2.5 text-sm leading-relaxed text-foreground shadow-md shadow-violet-500/10 backdrop-blur-[6px]">
      ...
      <div className="min-w-0 max-w-[88%] flex-1 sm:max-w-[82%]">
        ...
        <div className="mt-1 flex min-h-6 flex-wrap items-center gap-x-2 gap-y-1">
          ...
          {!streaming && msg.orquesta && (
            <span
              className="shrink-0 whitespace-nowrap rounded-full bg-forja-violet/12 px-1.5 text-[10px] font-medium text-forja-violet"
          ...
          {!streaming && (msg.content || msg.generatedImage) && (
            <div className="touch-actions ml-auto flex shrink-0 gap-0.5 opacity-100 transition md:opacity-0 md:group-hover:opacity-100">
              <IconBtn label={speaking ? "Detener lectura" : "Leer en voz alta"} onClick={toggleSpeak}>
                {speaking ? (
                  <VolumeX className="size-3.5 text-forja-violet" />
                ) : (
              ...
              {onGuardarRepaso && numTarjetasRepaso > 0 && (
                <IconBtn label="Guardar repaso" onClick={onGuardarRepaso}>
                  <GraduationCap className="size-3.5 text-forja-violet" />
                </IconBtn>
              )}
