...
import { fmtChars, useUsage } from "@/lib/forja/usage";
...
} from "@/lib/forja/gasto-modelos";
import { gastoDeHoy } from "@/lib/forja/chat-client";
import {
  PRECIOS_FECHA,
  costeDeModelo,
  cuantosSinPrecio,
  ...
  type Coste,
  type TablaPrecios,
} from "@/lib/forja/precios";
import { normalizarTope, TOPE_DIARIO_POR_DEFECTO } from "@/lib/forja/gasto";
...
function Barra({ parte }: { parte: number | null }) {
  ...
  return (
    <div className="mt-1 h-1.5 overflow-hidden rounded-full bg-muted">
      <div
        className="h-full rounded-full bg-forja-violet/70"
...
export function GastoPanelBody() {
  ...
  const tope = useForja((s) => normalizarTope(s.settings.topeLlamadasPago ?? TOPE_DIARIO_POR_DEFECTO));
  ...
  const filas = useMemo(
    () => filasDeGasto(byModel, (id) => PROVIDER_MAP[id as ProviderId]?.name ?? id),
    [byModel]
  );
