...
.prism-editable-hover{ outline: 2px dashed color-mix(in oklch, currentColor 60%, transparent) !important; outline-offset: 2px; cursor: text !important; }
.prism-editing{ outline: 2px solid #2563eb !important; outline-offset: 2px; background: color-mix(in oklch, #2563eb 8%, transparent) !important; }
.prism-texto-hover-caja{ position: fixed; pointer-events: none; z-index: 2147483647; outline: 2px dashed color-mix(in oklch, currentColor 60%, transparent); outline-offset: 1px; border-radius: 2px; }
`;
...
if (window.__prismEdit) return;
window.__prismEdit = true;
var activo = false;
var actual = null;
var envuelto = false;
var textoOriginal = "";
...
function marcar(on){
  try { document.documentElement.classList.toggle("prism-editando", on); } catch(e){}
}

function limpiarHover(el){
  if (el && el.classList) el.classList.remove("prism-editable-hover");
}
...
function pintarMarcaTexto(nodo){
  ...
  try {
    ...
    for (var i = 0; i < rects.length; i++) {
      ...
      caja.className = "prism-texto-hover-caja";
...
function onOver(ev){
  if (!activo) return;
  if (elegible(ev.target)) ev.target.classList.add("prism-editable-hover");
}
...
function terminar(commit){
  ...
  el.classList.remove("prism-editing");
  ...
  if (huboCambio) {
    try { parent.postMessage({ source: "prism-edit", type: "cambio", original: textoOriginal, nuevo: nuevo }, "*"); } catch(e){}
  }
}
...
function editarNodoTexto(nodo){
  ...
  span.classList.add("prism-editing");
  span.focus();
  try {
    var rango = document.createRange();
...
function onClick(ev){
  ...
  if (elegible(el)) {
    ...
    el.classList.add("prism-editing");
    el.focus();
    try {
      var rango = document.createRange();
...
window.addEventListener("message", function(e){
  var d = e.data;
  if (!d || d.source !== "prism-edit-cmd") return;
  if (d.op === "toggle") {
    activo = !!d.on;
    marcar(activo);
    if (!activo) {
...
export function injectEditPilot(html: string): string {
  if (!html || html.includes("__prismEdit")) return html;
