 ...
 * Cada tema cambia las variables --forja-violet / --forja-cyan / --forja-pink
...
export function applyAccent(accent: string, customHex?: string): void {
  ...
  if (accent === ACCENT_CUSTOM) {
    ...
    if (triad) {
      root.style.setProperty("--forja-violet", triad.main);
      root.style.setProperty("--forja-cyan", triad.second);
      root.style.setProperty("--forja-pink", triad.third);
