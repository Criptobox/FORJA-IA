...
export function parseToolCallsFromChunk(
  ...
  try {
    if (protocol === "openai") {
      const choices = c.choices as { delta?: Record<string, unknown> }[] | undefined;
      const delta = choices?.[0]?.delta;
      const calls = delta?.tool_calls;
      if (!Array.isArray(calls)) return [];
      return calls.map((tc: Record<string, unknown>, _i: number) => {
        const fn = (tc.function ?? {}) as Record<string, unknown>;
        return {
          // Los fragmentos posteriores del MISMO tool_call llegan SIN id
          // (solo `index`): id vacío = «continuación de lo que ya hay».
          ...
          // `index` del wire OpenAI: identifica la llamada a la que pertenece
          // el fragmento cuando en un mismo delta viajan varias a la vez.
    ...
    if (protocol === "gemini") {
      const cands = c.candidates as { content?: { parts?: { functionCall?: { name?: string; args?: unknown } }[] } }[] | undefined;
...
export interface ToolCallLite {
  ...
  argsText: string;
  /** `index` del wire OpenAI: qué tool_call del turno es este fragmento.
   *  Ausente en protocolos que no lo traen (Anthropic, Gemini) y en el
