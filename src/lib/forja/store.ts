...
export function safeLocalStorage(): Storage {
  ...
  void localStorage.getItem("forja-ai-v1");
  return {
    ...
    setItem: (name: string, value: string): void => {
      try {
        ...
        console.warn(
          "[forja] No se pudo persistir el estado en localStorage (¿cuota llena?). " +
...
export const useForja = create<ForjaState>()(
  persist(
    (set, get) => ({
      sessions: [],
    ...
    }),
    {
      ...
      partialize: (st) => {
        ...
        // en `forja-vault-v1` (ver vault.ts). Aquí se dejan vacías.
        let vaultOn = false;
        try {
          vaultOn = !!localStorage.getItem("forja-vault-v1");
        } catch {
          /* ignore */
        }
      ...
      merge: (persisted, current) => {
        const p = (persisted ?? {}) as Partial<ForjaState>;
        return {
          ...current,
          ...p,
