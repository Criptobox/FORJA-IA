 ...
 * `passport.ts`). La UI la pinta `components/forja/wrapped-dialog.tsx`.
 */
...
export function modelLabel(modelKey: string): string {
  ...
  // PROVIDER_MAP es un diccionario por id: con PROVIDERS[split.providerId]
  // se indexaba el ARRAY con un string («groq» → undefined) y la etiqueta
  // enseñaba el id crudo en vez de «Groq · llama-3.3…».
  const prov = PROVIDER_MAP[split.providerId];
