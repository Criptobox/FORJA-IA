...
if (window.__forjaQA) return;
window.__forjaQA = true;
...
function responder(token){
  try { parent.postMessage({ type:"forja-qa-result", token: token, result: medir() }, "*"); } catch (e) {}
}
...
export function injectVisualQA(html: string): string {
  if (!html || html.includes("forja-qa-run")) return html;
...
function medirAncho(
  ...
  return new Promise((resolve) => {
    ...
    const onMsg = (e: MessageEvent) => {
      ...
      if (!d || d.type !== "forja-qa-result" || d.token !== token) return;
...
export function onQAAutoResult(cb: (r: QAResult) => void): () => void {
  const onMsg = (e: MessageEvent) => {
    const d = e.data as { type?: string; token?: number; result?: QAResult } | null;
    if (!d || d.type !== "forja-qa-result" || d.token !== 0 || !d.result) return;
    cb(d.result);
  };
  window.addEventListener("message", onMsg);
