...
export function accessCodeHeaders(): Record<string, string> {
  const code = useForja.getState().settings.accessCode?.trim();
  return code ? { "x-forja-code": code } : {};
}
...
const PROXY_HEADERS = ["x-target-url", "x-forja-code"];
...
export async function streamChat(opts: StreamOptions): Promise<string> {
  ...
  const accumToolCalls = (chunk: unknown): void => {
    ...
    for (const lite of lites) {
      // Clave del acumulador: con el wire OpenAI, `index` identifica la
      // llamada a la que pertenece el fragmento — Groq y OpenRouter trocean
      // `arguments` en varios deltas SIN repetir el id, y el primer delta
      // puede traer varias llamadas a la vez (cada una con su index). Sin
      // index (Anthropic, Gemini, mensaje completo no-streaming) se cae al
      // comportamiento de siempre: la parte numérica del id, o ranura nueva.
      const key =
        typeof lite.index === "number"
          ? lite.index
          : Number(lite.id.replace(/\D/g, "")) || toolCallsAcc.size;
      const existing = toolCallsAcc.get(key);
      if (existing && (!lite.id || existing.id === lite.id)) {
        // Fragmento de la misma llamada: se pega `argsText` (streaming
        // parcial). Un id vacío es «continuación»; si trae id distinto y
        // no es fragmento, cae abajo como llamada nueva.
