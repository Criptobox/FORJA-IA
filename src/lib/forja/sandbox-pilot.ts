 ...
 * recibe órdenes (`forja-pilot-cmd`) y responde (`forja-pilot-result`). Sin
...
${FX_ASENTAR}
if (window.__forjaPilot) return;
window.__forjaPilot = true;
...
function enumeraBotones(){
  ...
  for (var i = 0; i < todos.length; i++){
    ...
    __forjaBotones.push(el);
    out.push({
      i: __forjaBotones.length - 1,
...
function responde(token, ok, data){
  try { parent.postMessage({ type: "forja-pilot-result", token: token, ok: ok, data: data }, "*"); } catch (e) {}
}

window.addEventListener("message", function(e){
  var d = e.data;
  if (!d || d.type !== "forja-pilot-cmd" || typeof d.token !== "number") return;
  var cmd = d.cmd || {};
  if (cmd.op === "click"){
...
setTimeout(function(){ try { parent.postMessage({ type: "forja-pilot-ready" }, "*"); } catch (e) {} }, 100);
})();`;

/** Inyecta el runtime del piloto en el HTML de la vista previa (idempotente). */
export function injectPilot(html: string): string {
  if (!html || html.includes("forja-pilot-cmd")) return html;
...
export function enviarCmdPiloto(
  ...
  return new Promise((resolve) => {
    ...
    const onMsg = (e: MessageEvent) => {
      ...
      if (!d || d.type !== "forja-pilot-result" || d.token !== token) return;
      fin({ ok: !!d.ok, data: d.data ?? {} });
    };
    window.addEventListener("message", onMsg);
    try {
      win.postMessage({ type: "forja-pilot-cmd", token, cmd }, "*");
    } catch {
