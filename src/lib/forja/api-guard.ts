 ...
 *  - `FORJA_ACCESS_CODE` definido → hay que mandar `x-forja-code` con ese valor.
 *  - Rutas que tocan el disco del servidor (`/api/repos`) → en producción
 *    exigen `FORJA_ACCESS_CODE` sí o sí. Sin él la ruta queda apagada, porque
...
export interface GuardInput {
  ...
  host: string | null;
  /** cabecera x-forja-code de la petición */
  code: string | null;
  /** valor de FORJA_ACCESS_CODE en el entorno, si lo hay */
...
export function evaluateGuard(input: GuardInput): GuardOutcome {
  ...
  if (touchesDisk && isProduction && !configurado) {
    return {
      ...
      hint:
        ...
        "Para habilitarlo define la variable de entorno FORJA_ACCESS_CODE y ponla " +
  ...
  if (configurado && code?.trim() !== configurado) {
    return {
      ...
      hint: "Configúralo en Ajustes → Chat con el mismo valor que FORJA_ACCESS_CODE.",
    };
  }
...
export function guardRequest(
  ...
  return evaluateGuard({
    ...
    code: req.headers.get("x-forja-code"),
    accessCode: process.env.FORJA_ACCESS_CODE ?? null,
